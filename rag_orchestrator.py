...
class RAGOrchestrator:
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            "add_to_db",
            lambda state: "process_pdf" if state.get("pdf_files") else END
        )
    ...
    def _add_to_database(self, state: RAGState) -> dict:
        ...
        if success:
            ...
            print(f"Successfully added {current_pdf['name']} to database ({current_pdf['document_count']} documents)")
            return {
                "processed_pdfs": processed_pdfs,
                "current_pdf": None  # Clear current PDF after processing
            }
        else:
    ...
    def _retrieve_information(self, state: RAGState) -> dict:
        ...
        # Use query analysis to determine if we need filtering
        pdf_filter = state.get("query_analysis", {}).get("pdf_filter")
    
        # Retrieve documents with optional filtering
        retrieved_chunks = self.vector_db.query(
            state["query"],
            n_results=12,  # Get more results for better context
    ...
    def query(self, user_query: str, conversation_context: str = "") -> str:
        ...
        enhanced_query = f"Conversation context: {conversation_context}\n\nCurrent query: {user_query}"
