...
class RAGOrchestrator:
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        # Add edges for PDF processing
        workflow.add_edge("initialize", "process_pdf")
        workflow.add_conditional_edges(
            "process_pdf",
            lambda state: "process_pdf" if state["pdf_files"] else "analyze_query",
            {"process_pdf": "add_to_db", "analyze_query": "analyze_query"}
        )
        workflow.add_edge("add_to_db", "process_pdf")
    ...
    def _initialize_system(self, state: RAGState) -> dict:
        ...
        if not pdf_files:
            raise ValueError(f"No PDF files found in {state['pdf_directory']}")
    ...
    def _process_next_pdf(self, state: RAGState) -> dict:
        ...
        print(f"Processing {pdf_file} ({pdf_index + 1}/{len(state['pdf_files'])})...")
    ...
    def _retrieve_information(self, state: RAGState) -> dict:
        ...
        retrieved_chunks = self.vector_db.query(
            state["query"],
            n_results=5,
            pdf_name=query_analysis["pdf_filter"],
            is_fda=query_analysis["is_fda_specific"] if query_analysis["is_fda_specific"] else None
        )
    ...
    def query(self, user_query: str) -> str:
        """Execute a query against the loaded documents"""
        query_state = {
            "pdf_directory": self.pdf_directory,
            "pdf_files": [],
            "processed_pdfs": [],
        ...
        result = self.workflow.invoke(query_state, from_node="analyze_query")
